...
export class FunctionInfo implements IFunctionInfo {
  ...
  private _parseNode(node: ASTNode): {
    ...
    switch (node.type) {
      ...
      case 'TSMethodSignature':
        ...
        kind = (node as any).kind === 'constructor' ? 'constructor' :
               (node as any).kind === 'get' ? 'getter' :
               (node as any).kind === 'set' ? 'setter' : 'method';
      ...
      case 'PropertyDefinition':
        ...
        kind = (node as any).kind === 'get' ? 'getter' :
               (node as any).kind === 'set' ? 'setter' : 'method';
        isStatic = (node as any).static === true;
        syntax = this._parseNode(funcNode).syntax;
        break;
  ...
  get wrapper(): ASTNode | null { return this._wrapper; }
  get engine(): string       { return this._engine; }
  get kind(): FunctionKind   { return this._kind; }
  get syntax(): FunctionSyntax { return this._syntax; }
  get isStatic(): boolean    { return this._static; }
